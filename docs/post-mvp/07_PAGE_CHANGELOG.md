...
- Dal 2026-09-21 l'icona dell'app e il `theme-color` usano `#2f5fa0`, lo stesso blu di `--primary` e `--accent-blue`, a sua volta misurato sull'inchiostro `#3060a0` del marchio CVC. Il `#063b52` precedente era un terzo blu allineato a nulla. I tre PNG del launcher sono rigenerati dal file SVG, così sorgente e icone non possono più divergere. Il marchio derivato finale resta rinviato: questo è un allineamento di colore, non un marchio nuovo.
...
- Dal 2026-09-20 l'indicatore di sesso è una figura e non più una lettera, così l'unica `M` della riga è il badge rosso del minorenne. Sotto i 380 px la figura scende nella riga dei dettagli, dove non toglie larghezza al nome.
- Le tre figure sono facce di **Material Symbols** (Google, Apache 2.0): `face_6` capelli corti con frangia per Uomo, `face_3` capelli lunghi per Donna, `face` senza capelli per Altro. L'unica modifica è la barra degli occhiali da sole al posto degli occhi, identica su tutte e tre: così l'unica differenza fra le figure sono i capelli. Due tentativi disegnati a mano erano stati scartati prima — i simboli di Marte e Venere si leggono come simboli e non come persone, e le sagome fatte di cerchio più spalle non reggevano l'ingrandimento. L'attribuzione sta in `src/components/PersonBadges.tsx` e nel README; non si aggiunge nessuna dipendenza.
...
- Dal 2026-09-21 i due marchi First differiscono **soltanto** per le cifre, su richiesta del proprietario (`0_3_0_OWNER_BRIEF.md` 1.6, riferimento `brief-0.3.0/first-marks-reference.png`). Base comune: tavola, cavalluccio e scritta `FIRST` di `first-27.png`. Le cifre sono quelle di `first-25-7.png` alla loro misura nativa (14 px), allineate a destra sul bordo destro della scritta. Cancellare il `.7` aveva lasciato un `27` alto 8 px contro i 14 dell'altro marchio: era questo a farli sembrare due marchi diversi. La differenza fra i due file misura 543 pixel, tutti dentro il gruppo delle cifre.
- A 320 px il trattamento dell'identità si compatta per lasciare interamente visibili anche i numeri a due cifre (11, 14, 15).
- Il riquadro del marchio va misurato sull'immagine, non con una percentuale: la riga della griglia è dimensionata sul contenuto, quindi `h-full` non aveva nulla di definito a cui riferirsi e ogni marchio tornava alla sua altezza nativa di 60 px dentro una riga da 36 px. Un marchio largo restava comunque limitato dagli 88 px della colonna e sembrava corretto — per questo il difetto è arrivato fino all'app: l'unica flotta mai provata era di RS Quest. RS 500, J/80 e i due First uscivano invece dalla card. Dal 2026-09-20 l'immagine porta `36 × 88` con `object-contain`, e `ug1-boat-mark-fit.spec.ts` misura tutti e sette i tipi canonici nel proprio riquadro.
...
- Sempre dal 2026-09-20 la card è più bassa di un quarto — da 72 px a 54 px minimi, con metà del padding verticale e un filo meno di distanza fra una card e l'altra. Il riquadro del marchio resta 36 px: quello che se ne va è soltanto l'aria attorno, e in una schermata entrano sette barche dove prima ne entravano cinque. `ug1-boat-mark-fit.spec.ts` misura anche questa altezza, così non torna a crescere per caso.
- Dal 2026-09-20 lo stato apre la riga, subito dopo il filo colorato e nello stesso blu: icona con la spunta cerchiata ed etichetta scritta, in una colonna di larghezza fissa, così la disponibilità della flotta si legge lungo una sola colonna mentre si scorre e numeri e marchi restano comunque allineati fra righe. Sotto i 380 px la colonna si libera e l'identità va a capo, come già faceva.
...
- Quando un equipaggio non ha una barca assegnata ma tutte le barche scelte per l'uscita sono dello stesso modello, la card mostra il marchio del modello accanto a “Senza barca”: si esce in quel modello, lo scafo non è ancora deciso. È lo stato “modello senza numero” del paragrafo 7.5 di `01`, e il workspace riusa il componente della vista di lettura. **Decisione dell'autore del 2026-09-20: il comportamento resta anche in P14**, perché dice quale modello esce; non era però leggibile. Da quella data il marchio “previsto” è attenuato (grigio, come ogni resa non definitiva dell'app) e anche la scritta “Senza barca” passa al grigio secondario, così una barca assegnata — marchio a colori e numero accanto — non si confonde con un equipaggio che conosce solo il modello.
- Il numero di equipaggi si sceglie prima di comporre, ma l'uscita cambia forma dopo: un volontario che non si imbarca lascia un equipaggio vuoto per tutta la sessione. Dal 2026-09-20 un equipaggio **vuoto** porta sulla propria card, a destra dei posti liberi, il comando che lo elimina; un equipaggio che contiene qualcuno non lo mostra affatto, così non si perde nessuno per un tocco. In fondo alla lista `Aggiungi equipaggio` rifà il gesto opposto entro il limite delle persone disponibili, senza tornare alla schermata di preparazione. Le due operazioni sono locali alla sessione: A terra, barche in uscita e le altre sessioni non cambiano.
